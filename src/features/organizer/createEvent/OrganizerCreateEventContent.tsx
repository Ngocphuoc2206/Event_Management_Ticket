...
function buildDraftPayload(form: FormState): OrganizerCreateEventPayload {
  ...
  return {
    ...
    shortDescription:
      form.description.trim().slice(0, 160) || "Organizer draft",
...
export function OrganizerCreateEventContent() {
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      showToast({
        tone: "success",
        message: "Draft đã được lưu lên hệ thống.",
      });
    } catch (error) {
      showToast({
        tone: "error",
        ...
        <div className="w-full space-y-10">
          <section className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
            <div className="max-w-160 space-y-2">
              <h1 className="text-4xl font-bold leading-12 text-zinc-900 sm:text-5xl">
                Create New Event
              </h1>
              <p className="text-lg leading-7 text-gray-700">
                Lưu Draft ở bước này để tạo event thật và tiếp tục hoàn thiện.
              </p>
            </div>
