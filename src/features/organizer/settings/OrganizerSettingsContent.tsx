...
function Toggle({ enabled, onClick }: { enabled: boolean; onClick: () => void }) {
  return (
    <button
      ...
      className={`relative h-7 w-12 rounded-full transition ${enabled ? "bg-sky-700" : "bg-slate-300"}`}
    >
      <span
    ...
    <section className="flex-1 bg-slate-50 text-zinc-900">
      <header className="flex min-h-20 w-full flex-wrap items-center justify-between gap-4 border-b border-slate-100 bg-white/80 px-8 py-4 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-3 sm:gap-6">
          ...
          <button type="button" className="rounded-full bg-sky-700/10 p-2 text-sky-700" aria-label="Settings">
      ...
      <div className="mx-auto flex w-full max-w-[1280px] flex-col gap-8 p-5 sm:p-8 lg:p-10">
        <section className="flex flex-wrap items-end justify-between gap-4">
          <div className="space-y-2">
            <p className="text-xs font-bold uppercase tracking-[2.4px] text-sky-700">Control Center</p>
            <h1 className="text-4xl font-bold leading-10 text-zinc-900">Organizer Settings</h1>
            <p className="max-w-2xl text-sm text-gray-700">Manage profile data, communication channels, and security policies for your organizer workspace.</p>
          </div>
        ...
        <section className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
          <article className="rounded-[32px] bg-white p-8 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] border border-slate-100">
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-xl font-bold text-zinc-900">Public Profile</h2>
              <button type="button" className="inline-flex items-center gap-2 rounded-xl bg-gray-100 px-3 py-2 text-xs font-semibold text-gray-700">
          ...
          <article className="rounded-[32px] bg-white p-8 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] border border-slate-100">
                ...
                return (
                  <div key={feature} className="flex items-center justify-between rounded-2xl bg-gray-100 px-4 py-3">
                    <div className="flex items-start gap-3">
                      <div className="mt-0.5 rounded-full bg-violet-700/10 p-2 text-violet-700">
            ...
            <button
              type="button"
              className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-zinc-900 px-4 py-3 text-sm font-semibold text-white"
            >
        ...
        <section className="rounded-[32px] bg-white p-8 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] border border-slate-100">
            ...
            {Object.entries(CHANNEL_LABELS).map(([key, label]) => {
              ...
              return (
                <article key={channel} className="rounded-2xl bg-gray-100 p-4">
                  <div className="mb-5 inline-flex rounded-full bg-sky-700/10 p-2 text-sky-700">{icon}</div>
