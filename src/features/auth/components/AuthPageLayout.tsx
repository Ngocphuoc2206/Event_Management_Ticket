...
import { AUTH_FOOTER_LINKS } from "@/features/auth/constants";
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#f8f9ff] text-slate-900">
      ...
      <div className="relative mx-auto flex min-h-screen max-w-[1600px] flex-col">
        {headerAction ? (
          <header className="flex items-center justify-between border-b border-white/60 bg-white/70 px-6 py-5 backdrop-blur-xl sm:px-8">
        ...
        <section className={`flex flex-1 flex-col items-center justify-center px-4 sm:px-6 ${topSpacingClassName}`}>
          {!headerAction ? (
            <Link href={logoHref} className="mb-10 flex items-center gap-3">
              {logo}
              <span className="text-[2rem] font-bold tracking-tight text-slate-900 sm:text-3xl">
                {logoText}
              </span>
            </Link>
          ) : null}
