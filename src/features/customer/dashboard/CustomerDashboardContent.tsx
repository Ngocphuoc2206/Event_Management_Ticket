...
function StatIcon({ icon, accent }: Pick<CustomerStatCard, "icon" | "accent">) {
  return (
    <div
      className={`flex h-9 w-9 items-center justify-center rounded-xl bg-slate-50 ${accent}`}
    >
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex h-full flex-col items-center justify-center px-5 text-center">
        {!imageSrc ? (
          <div className="text-[11px] uppercase tracking-[0.48em] text-slate-500/80">
            {artTitle}
          </div>
        ) : null}
        {!imageSrc ? (
...
function OrderStatusBadge({ status }: { status: CustomerOrderStatus }) {
  return (
    <span
      className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-semibold ${STATUS_STYLES[status]}`}
    >
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {statCards.map((card) => (
              ...
              >
                ...
                <div
                  className={`mt-6 text-[2rem] font-bold leading-none ${card.accent}`}
                >
                  {card.value}
                </div>
            ...
            {upcomingTickets.map((ticket) => (
              ...
              >
                ...
                <div className="flex flex-col p-5">
                  <h3 className="text-[1.8rem] font-bold leading-tight tracking-tight text-slate-900">
                    {ticket.title}
                  </h3>
        ...
        >
          ...
          >
            ...
            <div className="mt-6 rounded-[26px] bg-slate-50 p-5 text-center">
              ...
              <div className="mt-5 text-[10px] font-bold uppercase tracking-[0.24em] text-slate-400">
                Ticket Code
              </div>
              <div className="mt-2 text-sm font-semibold tracking-[0.24em] text-slate-700">
