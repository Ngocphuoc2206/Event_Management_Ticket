...
import { 
  Building2, BadgeCheck, ShieldAlert, DollarSign, Calendar,
    ...
    <AdminLayout title=""> 
      <div className="flex flex-col gap-8 animate-in fade-in duration-500 pb-10 relative">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl font-black text-slate-900 tracking-tight">Organizer Directory</h1>
            <p className="text-sm text-slate-500 font-medium mt-1.5">
              Verify organizers, monitor platform partners, and manage account statuses.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="bg-gradient-to-br from-indigo-900 to-indigo-950 p-8 rounded-[32px] text-white relative overflow-hidden shadow-xl shadow-indigo-200/50 group">
            <div className="relative z-10 flex flex-col h-full justify-between">
              <div>
                ...
                <h3 className="text-xl font-black mb-1.5 leading-tight text-white">Total Organizers</h3>
                <p className="text-indigo-200/80 text-[11px] font-bold uppercase tracking-widest mb-6">Registered entities</p>
              </div>
              <span className="text-4xl font-black text-white tracking-tight">1,248</span>
            </div>
          ...
          <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm flex flex-col justify-between">
            <div>
              ...
              <h3 className="text-xl font-black text-slate-900 mb-1.5">Verified Partners</h3>
              <p className="text-slate-400 text-[11px] font-bold uppercase tracking-widest mb-6">KYC Approved</p>
            </div>
            <span className="text-4xl font-black text-slate-900 tracking-tight">856</span>
          </div>
          ...
          <div className="bg-gradient-to-b from-rose-50 to-white p-8 rounded-[32px] border border-rose-100 flex flex-col justify-between">
            <div>
              ...
              <h3 className="text-xl font-black text-rose-950 mb-1.5">Pending Review</h3>
              <p className="text-rose-700/60 text-[11px] font-bold uppercase tracking-widest mb-6">Action Required</p>
            </div>
            <div className="flex items-baseline gap-2">
              <span className="text-4xl font-black text-rose-600 tracking-tight">14</span>
        ...
        <div className="bg-white rounded-[32px] border border-slate-200 shadow-sm overflow-hidden">
          <div className="flex justify-between items-center p-8 pb-6 border-b border-slate-100">
            <h3 className="text-xl font-black text-slate-900 tracking-tight">Partner List</h3>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 text-slate-400 text-[11px] font-black uppercase tracking-widest">
                  ...
                  <th className="py-5 px-4 text-center font-black">Events Hosted</th>
                  <th className="py-5 px-4 text-center font-black">Gross Revenue</th>
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {ORGANIZERS.map((org) => (
                  <tr key={org.id} className="group hover:bg-slate-50/70 transition-colors duration-200">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col justify-center">
                          <p className="font-bold text-slate-900 group-hover:text-indigo-600 transition-colors text-sm flex items-center gap-1.5">
                            {org.name}
                            {org.status === 'VERIFIED' && <BadgeCheck size={14} className="text-blue-500" fill="#eff6ff" />}
                          </p>
                          <p className="text-[11px] text-slate-500 font-semibold mt-1">{org.email}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-5 px-4 text-center">
                      <span className="inline-flex items-center gap-1.5 text-sm font-black text-slate-800 bg-slate-100 px-3 py-1 rounded-lg">
                        <Calendar size={14} className="text-slate-400"/> {org.eventsCount}
                      </span>
                    </td>
                    <td className="py-5 px-4 text-center">
                      <p className="text-base font-black text-emerald-600 tracking-tight">{org.totalRevenue}</p>
                    </td>
                    <td className="py-5 px-8 text-center">
                      <span className={`px-3.5 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border inline-flex items-center gap-1.5 ${
                        org.status === 'VERIFIED' ? 'bg-blue-50 text-blue-700 border-blue-200' :
                        org.status === 'PENDING_KYC' ? 'bg-amber-50/50 text-amber-700 border-amber-200' :
                        org.status === 'SUSPENDED' ? 'bg-rose-50/50 text-rose-700 border-rose-200' :
                        'bg-slate-50 text-slate-600 border-slate-200'
                      }`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${
                          org.status === 'VERIFIED' ? 'bg-blue-500' :
                          org.status === 'PENDING_KYC' ? 'bg-amber-500' :
