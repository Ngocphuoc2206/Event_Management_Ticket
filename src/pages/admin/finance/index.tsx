...
export default function FinanceManagementPage() {
  ...
  const handleActionPayout = (id: string, action: "COMPLETED" | "REJECTED") => {
    setTransactions((prev) =>
      prev.map((tx) => (tx.id === id ? { ...tx, status: action } : tx)),
    );
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm flex flex-col justify-between transition-shadow hover:shadow-md">
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-sm font-bold text-emerald-500 flex items-center bg-emerald-50 px-2 py-0.5 rounded-full">
          ...
          <div className="bg-gradient-to-b from-amber-50 to-white p-8 rounded-[32px] border border-amber-100 flex flex-col justify-between transition-shadow hover:shadow-md">
            <div>
              ...
              <h3 className="text-xl font-black text-amber-950 mb-1.5 leading-tight">
                Pending Payouts
              </h3>
              <p className="text-amber-700/60 text-[11px] font-bold uppercase tracking-widest mb-6">
                Owed to Organizers
              </p>
            </div>
            <div className="flex items-baseline gap-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-white rounded-[32px] border border-slate-100 shadow-sm p-8">
            ...
            <div className="h-64 flex items-end justify-between gap-2 md:gap-6 mt-4 px-2">
              {[
                { label: "Mon", in: 40, out: 20 },
                { label: "Tue", in: 70, out: 30 },
                { label: "Wed", in: 45, out: 15 },
                { label: "Thu", in: 90, out: 40 },
                { label: "Fri", in: 60, out: 25 },
                { label: "Sat", in: 100, out: 50 },
                ...
                >
                  <div className="w-full flex items-end justify-center gap-2 h-48 relative">
                    ...
                    <div
                      className="w-3 md:w-4 bg-indigo-200 group-hover:bg-indigo-300 transition-all duration-300 rounded-full z-10"
                      style={{ height: `${day.out}%` }}
                    ></div>
                    <div
                      className="w-3 md:w-4 bg-indigo-500 group-hover:bg-indigo-400 transition-all duration-300 rounded-full z-10 shadow-[0_4px_12px_rgba(99,102,241,0.25)] group-hover:shadow-[0_6px_16px_rgba(99,102,241,0.4)]"
                      style={{ height: `${day.in}%` }}
                    ></div>
                  </div>
          ...
          <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm p-8 flex flex-col">
            ...
            <div className="flex-1 flex flex-col justify-center">
              ...
              <div className="w-full h-3 rounded-full flex overflow-hidden mb-8 shadow-inner bg-slate-100">
                <div
                  className="bg-indigo-500 h-full shadow-[0_0_10px_rgba(99,102,241,0.3)] transition-all hover:brightness-110"
                  style={{ width: "65%" }}
                ></div>
                <div
                  className="bg-emerald-500 h-full shadow-[0_0_10px_rgba(16,185,129,0.3)] transition-all hover:brightness-110 border-l border-white/20"
                  style={{ width: "20%" }}
                ></div>
                <div
                  className="bg-amber-500 h-full shadow-[0_0_10px_rgba(245,158,11,0.3)] transition-all hover:brightness-110 border-l border-white/20"
                  style={{ width: "15%" }}
                ></div>
              </div>
            ...
            <button
              ...
              className={`px-6 py-3.5 border rounded-2xl font-black text-[11px] uppercase tracking-widest flex items-center gap-2 shrink-0 transition-all ${
                showFilters
                  ...
                  : "bg-white border-slate-200 text-slate-600 hover:text-indigo-600 hover:bg-slate-50 hover:border-slate-300 shadow-sm"
              }`}
            >
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {filteredTransactions.map((txn) => (
                  ...
                  >
                    ...
                    <td className="py-5 px-4 text-center">
                      <span className="text-[10px] font-black text-slate-500 bg-slate-100 px-2.5 py-1 rounded-md uppercase tracking-widest">
                        {txn.type}
                      </span>
                    </td>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
          ...
          <div className="relative bg-white w-full max-w-md rounded-[32px] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] overflow-hidden flex flex-col animate-in zoom-in-95 duration-300">
            <div className="flex justify-between items-center p-6 pb-4 border-b border-slate-100">
              ...
              <button
                onClick={() => setSelectedPayout(null)}
                className="w-9 h-9 flex items-center justify-center rounded-full bg-slate-50 text-slate-400 hover:bg-slate-100 hover:text-slate-700 transition-colors"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <div className="p-6 pt-4 flex gap-3 bg-white border-t border-slate-100">
              <button
                onClick={() =>
                  handleActionPayout(selectedPayout.id, "REJECTED")
                }
              ...
              <button
                onClick={() =>
                  handleActionPayout(selectedPayout.id, "COMPLETED")
                }
