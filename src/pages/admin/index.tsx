...
import { Users, Ticket, DollarSign, Activity, ArrowUpRight, TrendingUp, MoreHorizontal } from 'lucide-react';
...
const STATS = [
  ...
  { label: "Server Load", value: "42%", icon: Activity, trend: "Normal", color: "text-amber-600", bg: "bg-amber-50" },
];

const RECENT_TICKETS = [
  { id: "#TK-9921", event: "Coachella VIP Pass", organizer: "Goldenvoice", date: "2 mins ago", status: "Pending", amount: "$499" },
  { id: "#TK-9918", event: "AI Global Summit", organizer: "TechCorp", date: "15 mins ago", status: "Verified", amount: "$120" },
  { id: "#TK-9915", event: "Neon Night Fest", organizer: "Hoang Nam", date: "1 hour ago", status: "Verified", amount: "$45" },
  { id: "#TK-9912", event: "Startup Workshop", organizer: "Sarah Miller", date: "3 hours ago", status: "Rejected", amount: "$0" },
];
  ...
  return (
    <AdminLayout title="Dashboard Overview">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
        {/* TICKET PLATFORM PREVIEW */}
        <div className="xl:col-span-2 bg-white rounded-[40px] border border-slate-100 shadow-sm p-8">
          <div className="flex justify-between items-center mb-8">
            <div>
              <h3 className="text-xl font-black text-slate-900 tracking-tight">Recent Ticket Requests</h3>
              <p className="text-sm text-slate-400 font-medium">Monitoring Ticket Platform activities</p>
            </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] border-b border-slate-50">
                  ...
                  <th className="pb-5 text-right">Price</th>
                  <th className="pb-5"></th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {RECENT_TICKETS.map((tk, i) => (
                  <tr key={i} className="group hover:bg-slate-50/50 transition-all">
                    <td className="py-5">
                      <p className="font-black text-slate-900 text-sm group-hover:text-indigo-600 transition-colors">{tk.event}</p>
                      <p className="text-[11px] text-slate-400 font-bold">{tk.id} • {tk.date}</p>
                    </td>
                    <td className="py-5 text-center text-xs font-bold text-slate-600">{tk.organizer}</td>
                    <td className="py-5 text-center">
                      <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider ${
                        tk.status === 'Verified' ? 'bg-emerald-50 text-emerald-600' :
                        tk.status === 'Pending' ? 'bg-amber-50 text-amber-600' : 'bg-rose-50 text-rose-500'
                      }`}>
                        {tk.status}
                      </span>
                    </td>
                    <td className="py-5 text-right text-sm font-black text-slate-900">{tk.amount}</td>
